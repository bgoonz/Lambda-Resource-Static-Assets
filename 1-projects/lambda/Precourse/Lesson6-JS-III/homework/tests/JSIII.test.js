...
describe("returnLast(arr)", function () {
  it("should return the last item in the array", function () {
    ...
    expect(
      exercises.returnLast(["hi", "there", "how", "are", "you", "doing?"])
    ).toBe("doing?");
  });
});
...
describe("getArrayLength(arr)", function () {
  it("should return the length of the array", function () {
    ...
    expect(
      exercises.getArrayLength(["hi", "there", "how", "are", "you", "doing?"])
    ).toBe(6);
  });
});
...
describe("incrementByOne(arr)", function () {
  it("should return the array with each value incremented by one", function () {
    expect(exercises.incrementByOne([10, 10, 16, 12])).toEqual([
      11, 11, 17, 13,
    ]);
    expect(exercises.incrementByOne([97, 100, 80, 55, 72, 94])).toEqual([
      98, 101, 81, 56, 73, 95,
    ]);
...
describe("addItemToArray(arr, item)", function () {
  it("should return the array with the item added to the end", function () {
    expect(exercises.addItemToArray([10, 10, 16, 12], 10)).toEqual([
      10, 10, 16, 12, 10,
    ]);
    expect(
      exercises.addItemToArray([97, 100, 80, 55, 72, 94], "Hello")
    ).toEqual([97, 100, 80, 55, 72, 94, "Hello"]);
...
describe("addItemToFront(arr, item)", function () {
  it("should return the array with the item added to the front", function () {
    expect(exercises.addItemToFront([10, 10, 16, 12], 10)).toEqual([
      10, 10, 10, 16, 12,
    ]);
    expect(
      exercises.addItemToFront([97, 100, 80, 55, 72, 94], "Hello")
    ).toEqual(["Hello", 97, 100, 80, 55, 72, 94]);
...
describe("wordsToSentence(words)", function () {
  it("should return a string that has all of the words from the array separated by spaces", function () {
    expect(
      exercises.wordsToSentence(["LambdaSchool", "JavaScript", "Class"])
