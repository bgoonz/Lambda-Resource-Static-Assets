...
class gsuiDrumrows extends HTMLElement {
  ...
  stopRow(id) {
    this._rows
      .get(id)
      .root.querySelectorAll(".gsuiDrumrow-startCursor")
  ...
  add(id, elLine) {
    ...
    elRow.dataset.id = elLine.dataset.id = id;
    ...
    sliDetune.oninputend =
      sliGain.oninputend =
      sliPan.oninputend =
        this._oninputendRowSlider.bind(this, id);
  ...
  change(id, prop, val) {
    switch (prop) {
      case "pan":
        this._changePan(id, val);
        break;
      case "name":
        this._changeName(id, val);
        break;
      case "gain":
        this._changeGain(id, val);
        break;
      case "order":
        this._changeOrder(id, val);
        break;
      case "detune":
        this._changeDetune(id, val);
        break;
      case "toggle":
        this._changeToggle(id, val);
        break;
      case "pattern":
        this._changePattern(id, val);
        break;
      case "duration":
        this._changeDuration(id, val);
        break;
    }
  }
  _changePan(id, val) {
  ...
  _changeName(id, name) {
    ...
    el.dataset.name = el.textContent = name;
  }
  _changeToggle(id, b) {
  ...
  _changeDuration(id, dur) {
    this._rows
      .get(id)
      .root.querySelector(".gsuiDrumrow-waveWrap").style.animationDuration = `${
      dur * 2
    }s`;
  }
  _changePattern(id, svg) {
    const elWave = this._rows
      .get(id)
      .root.querySelector(".gsuiDrumrow-waveWrap");
  ...
  _changeOrder(id, order) {
    this._rows.get(id).root.dataset.order = this._lines.get(id).dataset.order =
      order;
  }
  ...
  _getPropBtn(rowId, prop) {
    return this._rows
      .get(rowId)
      .root.querySelector(
        `.gsuiDrumrow-propRadio[value="${prop}"] + .gsuiDrumrow-propSpan`
      );
  }
  ...
  _onclickRows(e) {
    if (e.target !== this) {
      ...
      switch (e.target.dataset.action) {
        case "props":
          this._expandProps(id);
          break;
        case "toggle":
          this._dispatch("change", "toggleDrumrow", id);
          break;
        case "delete":
          this._dispatch("change", "removeDrumrow", id);
          break;
      }
    }
  }
  _onmousedownRows(e) {
    if (
      ...
      e.target.classList.contains("gsuiDrumrow-main")
    ) {
      this._dispatch(
  ...
  _oncontextmenuRows(e) {
    ...
    if (e.target.dataset.action === "toggle") {
      this._dispatch(
        "change",
        "toggleSoloDrumrow",
        e.target.closest(".gsuiDrumrow").dataset.id
      );
  ...
  _ondragleaveRows() {
    if (this._elDragover) {
      ...
      this._elDragover = this._dragoverId = null;
    }
  }
  _ondragoverRows(e) {
