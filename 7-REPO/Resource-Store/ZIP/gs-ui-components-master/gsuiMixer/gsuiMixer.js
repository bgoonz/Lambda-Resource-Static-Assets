...
class gsuiMixer {
  constructor(opt) {
    ...
    this._analyserW = this._analyserH = 0;
  ...
  resized() {
    ...
    if (chans.length) {
      const { width, height } =
        chans[0].analyser.rootElement.getBoundingClientRect();
  ...
  addChannel(id, chan) {
    ...
    canvas.onclick = qs("nameWrap").onclick = this.selectChannel.bind(this, id);
    qs("toggle").onclick = () => this.onchange("toggleChannel", id);
    qs("delete").onclick = () => this.onchange("removeChannel", id);
    qs("connect").onclick = () =>
      this.onchange("redirectChannel", this._chanSelected, id);
