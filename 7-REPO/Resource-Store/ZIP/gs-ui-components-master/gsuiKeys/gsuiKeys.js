...
class gsuiKeys extends HTMLElement {
  constructor() {
    ...
    this._nbOct =
      ...
      this._keyIndMouse =
        0;
    this.onkeyup = this.onkeydown = this._elKeyMouse = null;
  ...
  octaves(start, nbOct) {
    ...
    Array.prototype.reduce.call(
      ...
      (midi, elKey, i) => {
        ...
        elKey.dataset.midi = elRow.dataset.midi = midi - 1;
        elKey.style.top = elRow.style.top = `${i}em`;
        return midi - 1;
      },
      maxOct * 12
    );
  ...
  getKeyElementFromMidi(midi) {
    return this.children[
      this.children.length - 1 - (midi - this._octStart * 12)
    ];
  }
  getMidiKeyFromKeyboard(e) {
    const k = gsuiKeys.keyboardToKey[e.code];

    return k ? (4 + k[0]) * 12 + k[1] : false;
  }
  midiReleaseAllKeys() {
  ...
  _evmuRoot() {
    ...
    if (this._elKeyMouse) {
      ...
      this._elKeyMouse = this._keyIndMouse = null;
    }
    this._gain = 1;
  }
  _evmmRoot(e) {
    ...
    if (e.clientX > this._blackKeyR && this._isBlack(~~(iKeyInd % 12))) {
      iKeyInd += fKeyInd - iKeyInd < 0.5 ? -1 : 1;
    }
