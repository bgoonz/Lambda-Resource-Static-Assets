...
class gsuiLFO extends HTMLElement {
  ...
  #children = GSUI.getTemplate("gsui-lfo");
    ...
    sliders: {
      delay: [
        ".gsuiLFO-delay  gsui-slider",
        ".gsuiLFO-delay  .gsuiLFO-propValue",
      ],
      attack: [
        ".gsuiLFO-attack gsui-slider",
        ".gsuiLFO-attack .gsuiLFO-propValue",
      ],
      speed: [
        ".gsuiLFO-speed  gsui-slider",
        ".gsuiLFO-speed  .gsuiLFO-propValue",
      ],
      amp: [
        ".gsuiLFO-amp    gsui-slider",
  ...
  updateWave(prop, val) {
    ...
    w.duration = this.#dur = Math.max(w.delay + w.attack + 2, bPM);
    w.draw();
  ...
  #changeToggle(b) {
    ...
    this.querySelectorAll(".gsuiLFO-typeRadio").forEach(
      b
  ...
  #changeAmpSign(amp) {
    this.querySelector(
      `.gsuiLFO-ampSignRadio[value="${Math.sign(amp) || 1}"]`
    ).checked = true;
  }
  #changeProp(prop, val) {
  ...
  // .........................................................................
  #updatePxPerBeat() {
    this.#elements.beatlines.setAttribute(
      "pxPerBeat",
      this.#waveWidth / this.#dur
    );
  }
  #initSlider(prop) {
    ...
    switch (e.target.name) {
      case "gsuiLFO-toggle":
        GSUI.setAttribute(
          this,
          "toggle",
          !this.classList.contains("gsuiLFO-enable")
        );
  ...
  #oninputSlider(prop, val) {
    const realval =
      prop !== "amp" ? val : val * Math.sign(this.getAttribute("amp"));
  ...
  #onchangeSlider(prop, val) {
    const nval =
      prop === "amp" ? val * Math.sign(this.getAttribute("amp")) : val;
