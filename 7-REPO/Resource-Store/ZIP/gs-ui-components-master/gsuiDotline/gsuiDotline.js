...
class gsuiDotline {
  constructor() {
    ...
    this._rootBCR = this._activeDot = this._attached = false;
    ...
    Object.entries(diff).forEach(([id, diffDot]) => {
      if (!diffDot) {
        ...
        const opt = this._opt,
          ...
          x =
            xs in diffDot
              ? this._epureNb(diffDot[xs], opt.minX, opt.maxX)
              : dot
              ? dot.x
              : 0,
          y =
            ys in diffDot
              ? this._epureNb(diffDot[ys], opt.minY, opt.maxY)
              : dot
              ? dot.y
              : 0;
  ...
  _drawPolyline() {
    const arr = [],
      ...
      svgW = this._svgW,
      svgH = this._svgH,
      { minX, minY, width, height, firstDotLinked, lastDotLinked } = this._opt;
    ...
    Object.entries(data).forEach(([id, { x, y }]) => {
      ...
      if (!newDot) {
        ...
        if (nx !== x || ny !== y) {
          ...
          obj[id] = objDot;
          if (nx !== x) {
            data[id].x = objDot[this._opt.x] = nx;
          }
          if (ny !== y) {
            data[id].y = objDot[this._opt.y] = ny;
          }
        }
      }
    });
  ...
  _epurePageY(py) {
    ...
    return (
      o.height - ((py - r.top - window.scrollY) / r.height) * o.height + o.minY
    );
  }
  _epureNb(n, min, max) {
  ...
  _mousedown(e) {
    ...
    if (e.button === 2) {
      ...
      if (this._opt.moveMode !== "linked") {
        ...
        this._dotMaxX = this._dotMaxY = -Infinity;
        this._dotMinX = this._dotMinY = Infinity;
