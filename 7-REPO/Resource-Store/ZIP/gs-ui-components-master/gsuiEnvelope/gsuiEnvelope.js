...
class gsuiEnvelope extends HTMLElement {
  ...
  #children = GSUI.getTemplate("gsui-envelope");
    ...
    sliders: {
      attack: [
        ".gsuiEnvelope-attack  gsui-slider",
        ".gsuiEnvelope-attack  .gsuiEnvelope-propValue",
      ],
      hold: [
        ".gsuiEnvelope-hold    gsui-slider",
        ".gsuiEnvelope-hold    .gsuiEnvelope-propValue",
      ],
      decay: [
        ".gsuiEnvelope-decay   gsui-slider",
        ".gsuiEnvelope-decay   .gsuiEnvelope-propValue",
      ],
      sustain: [
        ".gsuiEnvelope-sustain gsui-slider",
        ".gsuiEnvelope-sustain .gsuiEnvelope-propValue",
      ],
      release: [
        ".gsuiEnvelope-release gsui-slider",
        ".gsuiEnvelope-release .gsuiEnvelope-propValue",
      ],
    },
  });
  ...
  updateWave(prop, val) {
    ...
    g.duration = this.#dur = Math.max(
      g.attack + g.hold + g.decay + 0.5 + g.release,
      2
    );
    g.draw();
    ...
    switch (e.target.name) {
      case "gsuiEnvelope-toggle":
        GSUI.setAttribute(
          this,
          "toggle",
          !this.classList.contains("gsuiEnvelope-enable")
        );
