...
gsuiBlocksManager.prototype.__mouseup = function () {
  ...
  if (this.__status) {
    gsuiBlocksManager.__mouseupFns
      .get(this.__status)
      .call(this, blcsEditing, mdBlc);
  }
  this.__eventReset();
  if (mdBlc) {
    mdBlc.classList.remove("gsui-hover");
    this.__mdTarget.classList.remove("gsui-hover");
    this.__mdTarget = this.__mdBlc = null;
  }
...
gsuiBlocksManager.__mouseupFns = new Map([
  [
    "moving",
    function (blcsEditing) {
      if (this.__valueB || Math.abs(this.__valueA) > 0.000001) {
  ...
  [
    "cropping-a",
    function (blcsEditing) {
      if (Math.abs(this.__valueA) > 0.000001) {
