...
gsuiBlocksManager.__mousemoveFns = new Map([
  [
    ...
    function () {
      const croppingB = this.__status === "cropping-b",
        cropBrut =
          this.__beatSnap *
          Math.round((this.__mmWhen - this.__mdWhen) / this.__beatSnap),
  ...
  [
    ...
    function () {
      ...
      if (when !== this.__valueA) {
        this.__valueA = when;
        this.__blcsEditing.forEach((blc, id) =>
          this._blockDOMChange(blc, "when", this._data[id].when + when)
        );
      }
      if (rows !== this.__valueB) {
        this.__valueB = rows;
        this.__blcsEditing.forEach((blc) =>
          this._blockDOMChange(blc, "row", rows)
        );
      }
    },
  ],
  ...
  [
    ...
    function () {
      if (
        Math.abs(this.__mmPageX - this.__mdPageX) > 6 ||
  ...
  [
    ...
    function () {
      const rowH = this.__fontSize,
        ...
        duration =
          this.__getBeatSnap() + Math.abs(this.__mdWhen - this.__mmWhen),
      ...
      this.__blcsEditing.forEach((blc, id) =>
        this._blockDOMChange(blc, "selected", blcs.has(id))
      );
