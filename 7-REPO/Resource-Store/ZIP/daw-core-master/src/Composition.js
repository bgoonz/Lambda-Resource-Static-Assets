...
DAWCore.Composition = class {
  constructor(daw) {
    ...
    this.daw = daw;
    this.cmp = null;
    this.loaded = this.playing = false;
    this._saved = true;
    this._sched = sch;
    this._wamixer = wamix;
  ...
  load(cmpOri) {
    return new Promise((res, rej) => {
      ...
      Object.entries(cmp.buffers).forEach((kv) => {
        this.daw.buffers.setBuffer(kv[1]).then((buf) => {
          if (buf.buffer) {
      ...
      this._actionSavedOn = null;
      this._saved =
        cmp.options.saveMode === "cloud" ||
        DAWCore.LocalStorage.has(cmp.id) ||
        !cmp.savedAt;
  ...
  _onstopBlock(startedId) {
    const objStarted =
      this._startedSched.get(startedId) || this._startedBuffers.get(startedId);
