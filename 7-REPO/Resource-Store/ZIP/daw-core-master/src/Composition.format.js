...
DAWCore.Composition.format = function (cmp) {
  ...
  cmp.patternKeysOpened = cmp.patternKeysOpened
    ? `${cmp.patternKeysOpened}`
    : null;
  cmp.patternBufferOpened = cmp.patternBufferOpened
    ? `${cmp.patternBufferOpened}`
    : null;
  ...
  if (
    (!cmp.savedAt || cmp.savedAt < 1574550000) &&
    cmp.channels.main.gain > 0.8
  ) {
    // Sun Nov 24 2019 00:00:00 GMT+0100
    cmp.channels.main.gain = 0.4;
  }
  ...
  Object.values(cmp.keys).forEach((keys) => {
    Object.values(keys).forEach((k) => {
      ...
      k.gain = DAWCore.utils.castToNumber(k.gain, 0.8, 0, 1, 2);
      ...
      if (typeof k.prev === "number") {
        k.prev += "";
      }
      if (typeof k.next === "number") {
        k.next += "";
      }
      k.prev = k.prev || null;
      k.next = k.next || null;
      ...
      if (typeof k.key === "string") {
        if (window.gsuiKeys) {
          ...
          console.warn(
            "DAWCore.Composition.format: gsuiKeys is needed to convert an old midi notation"
          );
          return null;
        }
      }
