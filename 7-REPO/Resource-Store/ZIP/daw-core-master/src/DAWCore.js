...
class DAWCore {
  constructor() {
    ...
    this.get = {
      ...
      audioBuffer: (id) =>
        this.buffers.getBuffer(this.composition.cmp.buffers[id]).buffer,
      ...
      drums: (id) =>
        id ? this.composition.cmp.drums[id] : this.composition.cmp.drums, // 1.
      effect: (id) => this.composition.cmp.effects[id],
      effects: () => this.composition.cmp.effects,
      keys: (id) =>
        id ? this.composition.cmp.keys[id] : this.composition.cmp.keys, // 1.
    ...
    this._wadrumrows.onstartdrumcut = (rowId) =>
      this._call("onstopdrumrow", rowId);
    this.setLoopRate(60);
  ...
  isPlaying() {
    return (
      this.composition.playing || this.pianoroll.playing || this.drums.playing
    );
  }
  togglePlay() {
