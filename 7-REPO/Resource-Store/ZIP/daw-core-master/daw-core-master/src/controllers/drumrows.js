...
DAWCore.controllers.drumrows = class {
  constructor(fns) {
    ...
    this.on = DAWCore.utils.mapCallbacks(
      ["addDrumrow", "removeDrumrow", "changeDrumrow"],
      fns.dataCallbacks
    );
    this._drumrowsCrud = DAWCore.utils.createUpdateDelete.bind(
      null,
      this.data.drumrows,
  ...
  change({ patterns, drumrows }) {
    if (patterns) {
      this._patternsCrud(patterns);
    }
    if (drumrows) {
      this._drumrowsCrud(drumrows);
    }
  }
  clear() {
    Object.keys(this.data.patterns).forEach(
      (id) => delete this.data.patterns[id]
    );
  ...
  _updatePattern(id, { name, duration, dest }) {
    if (name !== undefined || duration !== undefined || dest !== undefined) {
      const pat = this.data.patterns[id],
        rowsEnt = Object.entries(this.data.drumrows).filter(
          (kv) => kv[1].pattern === id
        );
