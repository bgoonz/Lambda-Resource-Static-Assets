...
DAWCore.controllers.drums = class {
  constructor(fns) {
    ...
    this.on = DAWCore.utils.mapCallbacks(
      ["addDrum", "removeDrum", "changeDrum", "addDrumcut", "removeDrumcut"],
      fns.dataCallbacks
    );
    this._drumsCrud = DAWCore.utils.createUpdateDelete.bind(
      null,
      this.data,
  ...
  _deleteDrum(id) {
    const fn =
      "gain" in this.data[id] ? this.on.removeDrum : this.on.removeDrumcut;
