...
DAWCore.actions.removePattern = (patId, get) => {
  ...
  if (DAWCore.utils.isntEmpty(blocks)) {
    const realDur = Object.values(get.blocks()).reduce((dur, blc) => {
  ...
  if (type === "keys") {
    if (patId === get.patternKeysOpened()) {
      if (
        !Object.entries(get.patterns()).some(([k, v]) => {
  ...
  return [obj, ["patterns", "removePattern", pat.type, pat.name]];
};
