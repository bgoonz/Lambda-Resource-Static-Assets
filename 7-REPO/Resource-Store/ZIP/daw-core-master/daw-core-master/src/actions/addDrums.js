...
DAWCore.actions.addDrums = (patternId, rowId, whenFrom, whenTo, get) => {
  return DAWCore.actions._addDrums(
    "drum",
    true,
    patternId,
    rowId,
...
DAWCore.actions._addDrums = (
  ...
  for (let w = whenA; w <= whenB; ++w) {
    ...
    if (drmId) {
      ...
      drumWhenMax = Math.max(drumWhenMax, when + 0.001);
  ...
  if (nbDrums > 0 && !status) {
    drumWhenMax = drumsEnt.reduce((dur, [drumId, drum]) => {
      return drumId in newDrums ? dur : Math.max(dur, drum.when + 0.001);
    }, 0);
  }
  if (nbDrums > 0) {
