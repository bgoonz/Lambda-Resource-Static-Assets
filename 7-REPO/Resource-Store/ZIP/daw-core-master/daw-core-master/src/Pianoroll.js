...
DAWCore.Pianoroll = class {
  constructor(daw) {
    ...
    this.daw = daw;
    this.keys = {};
    this.looping = this.playing = false;
    this._synth = this.loopA = this.loopB = null;
    this.duration = 0;
    this._waKeys = waKeys;
  ...
  openPattern(id) {
    ...
    id ? daw.pianorollFocus() : daw.compositionFocus("-f");
    if (wasPlaying) {
      daw.stop();
      daw.stop();
    }
  ...
  clearLoop() {
    ...
    this.looping = false;
    this._waKeys.scheduler.setLoopBeat(
      0,
      this.duration || this.daw.get.beatsPerMeasure()
    );
  }
  liveKeydown(midi) {
    if (!(midi in this._keysStartedLive)) {
      this._keysStartedLive[midi] = this._synth.startKey(
        ...
        this._waKeys.scheduler.currentTime(),
        0,
        Infinity
      );
    }
