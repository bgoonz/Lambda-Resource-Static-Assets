...
class GSPatterns {
  constructor() {
    ...
    this._synthsCrud = DAWCore.utils.createUpdateDelete.bind(
      null,
      this.data.synths,
    ...
    this._patternsCrud = DAWCore.utils.createUpdateDelete.bind(
      null,
      this.data.patterns,
    ...
    this._channelsCrud = DAWCore.utils.createUpdateDelete.bind(
      null,
      this.data.channels,
      ...
      Object.entries(this._dawcore.get.patterns()).forEach(([id, pat]) => {
        if (
          (pat.type === "drums" &&
            (obj.drumrows ||
              obj.drums?.[pat.drums] ||
              obj.patterns?.[id]?.duration)) ||
          (pat.type === "keys" &&
            (obj.keys?.[pat.keys] || obj.patterns?.[id]?.duration))
        ) {
  ...
  _updatePatternContent(id) {
    ...
    if (elPat) {
      ...
      if (type !== "buffer") {
        this.svgForms[type].setSVGViewbox(
  ...
  _updateSynth(id, obj) {
    ...
    if ("dest" in obj) {
      this._uiPatterns.changeSynth(
        id,
        "destName",
        this._dawcore.get.channel(obj.dest).name
      );
    }
  }
  _deleteSynth(id) {
  ...
  _updatePattern(id, obj) {
    ...
    if ("dest" in obj) {
      this._uiPatterns.changePattern(
        id,
        "destName",
        this._dawcore.get.channel(obj.dest).name
      );
