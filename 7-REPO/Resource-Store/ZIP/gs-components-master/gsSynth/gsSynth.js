...
class GSSynth {
  constructor() {
      ...
      dataSynth = new DAWCore.controllers.synth({
        dataCallbacks: {
          ...
          changeOscProp: (id, k, v) =>
            GSUI.setAttribute(uiSynth.getOscillator(id), k, v),
    ...
    this._dawcore = this._synthId = null;
    Object.seal(this);
    ...
    uiSynth.addEventListener("gsuiEvents", (e) => {
      ...
      switch (d.component) {
        case "gsuiEnvelope":
          switch (d.eventName) {
            case "toggle":
              dc.callAction("toggleEnv", id);
              break;
            case "change":
              dc.callAction("changeEnv", id, ...a);
              break;
        ...
        case "gsuiLFO":
          switch (d.eventName) {
            case "toggle":
              dc.callAction("toggleLFO", id);
              break;
            case "change":
              dc.callAction("changeLFO", id, ...a);
              break;
            case "liveChange":
              dc.liveChangeSynth(id, { lfo: { [a[0]]: a[1] } });
              break;
          }
          break;
        case "gsuiSynthesizer":
          switch (d.eventName) {
            case "addOscillator":
              dc.callAction("addOscillator", id);
              break;
            case "reorderOscillator":
              dc.callAction("reorderOscillator", id, a[0]);
              break;
          }
          break;
        case "gsuiOscillator":
          {
            ...
            switch (d.eventName) {
              case "remove":
                dc.callAction("removeOscillator", id, oscId);
                break;
              case "change":
                dc.callAction("changeOscillator", id, oscId, ...a);
                break;
              case "liveChange":
                dc.liveChangeSynth(id, {
                  oscillators: { [oscId]: { [a[0]]: a[1] } },
                });
                break;
            }
          }
  ...
  change(obj) {
    ...
    if ("beatsPerMeasure" in obj || "stepsPerBeat" in obj) {
      GSUI.setAttribute(
        this.rootElement.env,
        "timedivision",
        `${get.beatsPerMeasure()}/${get.stepsPerBeat()}`
      );
      GSUI.setAttribute(
        this.rootElement.lfo,
        "timedivision",
        `${get.beatsPerMeasure()}/${get.stepsPerBeat()}`
      );
    }
