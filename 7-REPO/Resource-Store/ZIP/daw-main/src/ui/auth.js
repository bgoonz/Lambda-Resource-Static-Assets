...
function UIauthLoading(b) {
  DOM.login.dataset.spin = DOM.logout.dataset.spin = b ? "on" : "";
}
...
function UIauthLogoutThen() {
  ...
  Array.from(DOM.cmpsCloudList.children).forEach((el) =>
    DAW.deleteComposition("cloud", el.dataset.id)
  );
  if (!DAW.get.cmp()) {
...
function UIauthSaveComposition(cmp) {
  return gsapiClient.saveComposition(cmp).then(
    () => cmp,
    (err) => {
      gsuiPopup.alert(
        `Error ${err.code}`,
