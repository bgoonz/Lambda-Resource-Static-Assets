...
function UIrun() {
  ...
  DAW.cb.pause = DAW.cb.stop = () => (DOM.play.dataset.icon = "play");
  DAW.cb.play = () => (DOM.play.dataset.icon = "pause");

  DOM.versionNum.textContent = DOM.headVersionNum.textContent = window.VERSION;

  DOM.winBtnsMap.forEach(
    (btn, id) => id !== "effects" && id !== "drums" && btn.click()
  );
