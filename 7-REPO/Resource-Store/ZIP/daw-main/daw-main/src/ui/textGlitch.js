...
class TextGlitch {
  ...
  _glitch() {
    const clip1 = this._randDouble(0.2),
      clip2 = this._randDouble(0.2);

    this._elClips.forEach((el) => {
      const x = this._randDouble(0.25),
        y = this._randDouble(0.05);

      el.style.transform = `translate(${x}em, ${y}em)`;
    });
    this._elClips[0].style.clipPath = `inset( 0 0 ${0.6 + clip1}em 0 )`;
    this._elClips[1].style.clipPath = `inset( ${0.4 - clip1}em 0 ${
      0.3 - clip2
    }em 0 )`;
    this._elClips[2].style.clipPath = `inset( ${0.7 + clip2}em 0 0 0 )`;
