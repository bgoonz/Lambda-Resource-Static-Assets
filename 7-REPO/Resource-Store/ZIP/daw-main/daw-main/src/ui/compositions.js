...
function UIcompositionsInit() {
  DOM.cmpsCloudList.onclick = DOM.cmpsLocalList.onclick = UIcompositionClick;
...
function UIcompositionOpened(cmp) {
  ...
  par.scrollTop = 0;
  DOM.headCmp.dataset.saveMode = DOM.headCmpIcon.dataset.icon =
    cmp.options.saveMode;
  DOM.headCmpSave.dataset.icon =
    cmp.options.saveMode === "local" ? "save" : "upload";
...
function UIcompositionLoading(cmp, loading) {
  DOM.headCmpSave.dataset.spin = UIcompositions.get(cmp).save.dataset.spin =
    loading ? "on" : "";
}
...
function UIcompositionAdded(cmp) {
  ...
  if (html) {
    ...
    (local ? DOM.cmpsLocalList : DOM.cmpsCloudList).append(root);
  }
}
...
function UIcompositionClickNewLocal() {
  (!DAW.compositionNeedSave()
    ...
    : gsuiPopup
        .confirm("Warning", "Are you sure you want to discard unsaved works")
      ...
      if (b) {
        (saveMode === "cloud"
          ...
          .then(() => {
            if (id === DAW.get.id()) {
              for (
                let next = html.root.nextElementSibling;
                next;
