...
function UIcontrolsInit() {
  ...
  DOM.cmpsBtn.onmousedown = DOM.undoMore.onmousedown =
    UIcontrolsDropdownBtnClick;
  ...
  UIclock.onchangeDisplay = (mode) =>
    localStorage.setItem("gsuiClock.display", mode);
  GSUI.setAttribute(
    UIclock,
    "mode",
    localStorage.getItem("gsuiClock.display") || "second"
  );
}
...
function UIcontrolsClickCmp() {
  gsuiPopup
    .prompt("Composition's title", "", DAW.get.name(), "Rename")
...
function UIcontrolsFocusOn(focStr, b) {
  if (b) {
    ...
    UIpianoroll.rootElement.classList.toggle(
      "selected",
      focStr === "pianoroll"
    );
