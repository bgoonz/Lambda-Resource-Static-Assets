...
QUnit.module("DAW", () => {
  ...
  QUnit.test("Splashscreen: click", (assert) => {
    ...
    setTimeout(() => {
      assert.strictEqual(
        qS("#loading"),
        null,
        "Splashscreen clicked and removed."
      );
      done();
    }, 3000);
  });
  ...
  QUnit.test("Pianoroll: create a key", (assert) => {
    ...
    assert.strictEqual(
      DAW.get.keys(0)[1].when,
      1,
      "DAW.get.keys( 0 )[ 1 ].when"
    );
    assert.strictEqual(
      qSA(".historyAction").length,
      nbActions + 1,
      '$( ".historyAction" ).length'
    );
  });
  ...
  QUnit.test("Controls: clock", (assert) => {
    ...
    assert.strictEqual(
      __getClockContent(),
      "2:01:000",
      "beat:2 focus:pianoroll display:beat"
    );
    __changeClockMode();
    assert.strictEqual(
      __getClockContent(),
      "0:00:500",
      "beat:2 focus:pianoroll display:second"
    );
    DAW.compositionFocus();
    assert.strictEqual(
      __getClockContent(),
      "0:00:000",
      "beat:1 focus:composition display:second"
    );
    __changeClockMode();
    assert.strictEqual(
      __getClockContent(),
      "1:01:000",
      "beat:1 focus:composition display:beat"
    );
    ...
    assert.strictEqual(
      __getClockContent(),
      "3:01:000",
      "beat:3 focus:pianoroll display:beat"
    );
  });
