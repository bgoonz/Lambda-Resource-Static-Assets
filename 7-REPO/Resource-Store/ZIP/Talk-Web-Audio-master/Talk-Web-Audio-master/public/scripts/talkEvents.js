...
var line,
  ...
  xScale = d3.scale
    .linear()
    .domain([0, samples - 1])
    .range([0, svgDomDi.width - 50]),
  yScale = d3.scale
    .linear()
    .domain([-1, 1])
    .range([svgDomDi.height - 50, 0]),
...
svg
  .append("g")
  .attr("class", "x axis")
  .attr("transform", "translate(0," + (svgDomDi.height - 40) + ")")
  .call(xAxis)
  .append("text")
  .text("time (ms)")
  .attr("transform", "translate(" + (svgDomDi.width - 100) + ",-10)");
svg
  .append("g")
  .attr("class", "y axis")
  .call(yAxis)
      ...
      space.addEventListener("mousedown", function (evt) {
        ...
        this.addEventListener("mousemove", function (evt) {
          soundSource.setPosition(
            (evt.clientX - startPosX) / 10,
            (evt.clientY - startPosY) / 10,
            (evt.clientY - startPosY) / 10
          );
          spaceSource.style.transform =
            "translate(" +
            (evt.clientX - startPosX) +
            "px, " +
            (evt.clientY - startPosY) +
            "px)";
...
Reveal.addEventListener("white", function (ev) {
  window.addEventListener("keydown", function (evt) {
    if (evt.key === "g") {
      ...
      const whiteBuffer = context.createBuffer(
        1,
        whiteBufferSize,
        context.sampleRate
      );
...
Reveal.addEventListener("pink", function (ev) {
  window.addEventListener("keydown", function (evt) {
    if (evt.key === "h") {
      ...
      // pass in channels, frame count, sample rate
      const pinkBuffer = context.createBuffer(
        1,
        pinkBufferSize,
        context.sampleRate
      );
...
Reveal.addEventListener("vis_canvas", function (ev) {
