...
class gswaDrumrows {
  constructor() {
    ...
    this.ctx = this.onstartdrum = this.onstartdrumcut = null;
    this.getAudioBuffer = this.getChannelInput = DAWCore.utils.noop;
  ...
  startDrumcut(drumcut, when) {
    ...
    this._startedDrums.forEach((nodes, id) => {
      if (
        nodes.absn &&
        nodes.rowId === drumcut.row &&
        nodes.when < whenCutStart &&
        when < nodes.endAt
      ) {
        nodes.endAt = when;
        nodes.gainCut.gain.setValueCurveAtTime(
  ...
  _startDrum(drum, when, off, durUser, live) {
    ...
    if (buffer) {
      ...
      nodes.panRow = panRow;
      absn.buffer = buffer;
      absn.detune.setValueAtTime(
        (row.detune + drum.detune) * 100,
        this.ctx.currentTime
      );
      gainRow.gain.setValueAtTime(
        row.toggle ? row.gain * drum.gain : 0,
        this.ctx.currentTime
      );
      panRow.pan.setValueAtTime(
        DAWCore.utils.panningMerge(row.pan, drum.pan),
        this.ctx.currentTime
      );
      ...
      if (this.onstartdrum) {
        ...
        nodes.startDrumTimeoutId = setTimeout(
          () => this.onstartdrum(rowId),
          timeoutMs
        );
      }
    }
  ...
  changeDrumProp(id, prop, val) {
    ...
    if (nodes) {
      ...
      switch (prop) {
        case "detune":
          nodes.detune = val;
          nodes.absn.detune.setValueAtTime(
            (val + row.detune) * 100,
            this.ctx.currentTime
          );
          break;
        case "gain":
          nodes.gain = val;
          nodes.gainRow.gain.setValueAtTime(
            row.toggle ? val * row.gain : 0,
            this.ctx.currentTime
          );
          break;
        case "pan":
          {
            nodes.pan = val;
            nodes.panRow.pan.setValueAtTime(
              DAWCore.utils.panningMerge(val, row.pan),
              this.ctx.currentTime
            );
          }
          break;
  ...
  _changeDrumrow(id, prop, val) {
    ...
    switch (prop) {
      ...
      case "pan":
        this.__changeDrumrow(id, (nodes) => {
          nodes.panRow.pan.setValueAtTime(
            DAWCore.utils.panningMerge(val, nodes.pan),
            this.ctx.currentTime
          );
        });
        break;
