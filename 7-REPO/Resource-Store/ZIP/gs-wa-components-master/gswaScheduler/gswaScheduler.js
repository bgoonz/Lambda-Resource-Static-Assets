...
class gswaScheduler {
  constructor() {
    ...
    this._crud = DAWCore.utils.createUpdateDelete.bind(
      null,
      this.data,
  ...
  getFutureOffsetAt(futureTime) {
    ...
    if (this.looping && t > this.loopB - 0.001) {
      t = this.loopA + ((t - this.loopA) % this.loopDuration);
      if (t > this.loopB - 0.001) {
        t = this.loopA;
      }
    }
    return t;
  ...
  startBeat(when, off = 0, dur) {
    return this.start(
      when,
      off / this.bps,
      Number.isFinite(dur) ? dur / this.bps : dur
    );
  }
  start(when, off = 0, dur) {
    ...
    this._startOff = off;
    this._startDur = this._startFixedDur ? dur : this.duration - off;
    ...
    this.isStreaming ? this._streamloopOn() : this._fullStart();
  }
  stop() {
    if (this.started) {
  ...
  _fullStart() {
    ...
    this._sortedData.forEach((kv) =>
      this._blockStart(when, from, to, to, ...kv)
    );
  }
  ...
  _findLastBlock() {
    this._updateDuration(
      Object.entries(this.data).reduce((max, [id, blc]) => {
  ...
  change(obj) {
    this._crud(obj);
    this._sortedData = Object.entries(this.data).sort(
      (a, b) => a[1].when - b[1].when
    );
  }
  _dataDeleteBlock(id) {
    if (!(id in this.data)) {
  ...
  _dataUpdateBlockProp(id, prop, val) {
    const propTime =
        prop === "when" || prop === "offset" || prop === "duration",
    ...
    if (this.started) {
      if (propTime || propLink) {
        ...
        Object.keys(this._dataScheduledPerBlock[id].started).forEach(
          (startedId) => {
