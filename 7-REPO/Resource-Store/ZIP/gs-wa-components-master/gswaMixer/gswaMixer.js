...
class gswaMixer {
  constructor() {
    this.ctx = this.connectedTo = null;
    this._chans = {};
    this._fftSize = 4096;
  ...
  _addChan(id) {
    ...
    analyserL.fftSize = analyserR.fftSize = this._fftSize;
  ...
  _toggleChan(id, val) {
    this._chans[id].gain.gain.setValueAtTime(
      val ? this._ctrlMixer.data.channels[id].gain : 0,
