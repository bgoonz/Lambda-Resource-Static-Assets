...
class gswaLFO {
  constructor(ctx) {
    ...
    this.ctx = ctx;
    this.node = node;
    this._oscNode = this._ampNode = this._ampAttNode = null;
  ...
  start(d) {
    ...
    data.whenStop = d.whenStop
      ? Math.max(d.when + d.delay + d.attack + 0.1, d.whenStop)
      : 0;
  ...
  _setAmpAtt() {
    ...
    if (now <= atTime && d.attack > 0) {
      ...
      this._ampAttNode.gain.setValueCurveAtTime(
        new Float32Array([0, absAmp]),
        atTime,
        d.attack
      );
    } else {
  ...
  static _setVariations(d, absProp, prop, nodeParam, now) {
    ...
    d.variations.forEach((va) => {
      ...
      if (when > now && dur > 0) {
        ...
        nodeParam.setValueCurveAtTime(
          new Float32Array([absVal * ab[0], absVal * ab[1]]),
          when,
          dur
        );
      }
