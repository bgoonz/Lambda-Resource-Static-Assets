...
class gswaSynth {
  ...
  _addOsc(id, osc) {
    this._startedKeys.forEach((k) =>
      k.oscNodes.set(id, this._createOscNode(k, id))
    );
  }
  _changeOsc(id, obj) {
    ...
    this._startedKeys.forEach((key) => {
      ...
      objEnt.forEach(([prop, val]) => {
        switch (prop) {
          case "type":
            this._nodeOscSetType(nodes.oscNode, val);
            break;
          case "pan":
            nodes.panNode.pan.setValueAtTime(val, now);
            break;
          case "gain":
            nodes.gainNode.gain.setValueAtTime(val, now);
            break;
          case "detune":
            nodes.oscNode.detune.setValueAtTime(val * 100, now);
            break;
        }
      });
    });
  ...
  _changeEnv(obj) {
    this._startedKeys.forEach((key) => {
      ...
      if ("decay" in nobj) {
        nobj.decay /= this._bps;
      }
      if ("attack" in nobj) {
        nobj.attack /= this._bps;
      }
      if ("release" in nobj) {
        nobj.release /= this._bps;
      }
  ...
  startKey(blocks, when, off, dur) {
    const id = ++gswaSynth._startedMaxId.value,
      ...
      key = Object.freeze({
        ...
        attack: blc0.attack / bps || 0.005,
        release: blcLast.release / bps || 0.005,
        variations: [],
        oscNodes: new Map(),
    ...
    key.lowpassNode.frequency.setValueAtTime(
      this._calcLowpass(key.lowpass),
      atTime
    );
    key.highpassNode.frequency.setValueAtTime(
      this._calcHighpass(key.highpass),
      atTime
    );
    ...
    Object.keys(oscs).forEach((id) =>
      key.oscNodes.set(id, this._createOscNode(key, id))
    );
  ...
  stopKey(id) {
    ...
    if (key) {
      if (Number.isFinite(key.dur)) {
        ...
        setTimeout(
          this._stopKey.bind(this, id),
          ((this.gsdata.data.env.release + 0.1) / this._bps) * 1000
        );
      }
    } else {
  ...
  _calcExp(x, total, exp) {
    return exp === 0 ? x : (Math.expm1(x) ** exp / (Math.E - 1) ** exp) * total;
  }
  ...
  _scheduleVariations(key) {
    key.variations.forEach((va) => {
      ...
      if (when > this.ctx.currentTime && dur > 0) {
        key.oscNodes.forEach((nodes) =>
          nodes.oscNode.frequency.setValueCurveAtTime(freqArr, when, dur)
        );
        key.panNode.pan.setValueCurveAtTime(
          new Float32Array(va.pan),
          when,
          dur
        );
        key.gainNode.gain.setValueCurveAtTime(
          new Float32Array(va.gain),
          when,
          dur
        );
        key.lowpassNode.frequency.setValueCurveAtTime(
          new Float32Array(va.lowpass),
          when,
          dur
        );
        key.highpassNode.frequency.setValueCurveAtTime(
          new Float32Array(va.highpass),
          when,
          dur
        );
      }
  ...
  _createOscNode(key, id) {
    ...
    oscNode.connect(panNode).connect(gainNode).connect(key.LFONode.node);
...
gswaSynth.nativeTypes = Object.freeze([
  "sine",
  "triangle",
  "sawtooth",
  "square",
